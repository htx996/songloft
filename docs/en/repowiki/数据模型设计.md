...
The current schema has a total of **8 tables**: `songs`, `playlists`, `playlist_songs`, `play_history`, `configs`, `auth_tokens`, `js_plugins`, and `plugin_storage`, evolved from the initial version to the current state through 35 migration files. The `updated_at` column of the core tables is automatically maintained by a SQLite TRIGGER.
...
erDiagram
    ...
    play_history {
        INTEGER id PK
        TEXT context_type "playlist | artist | album | genre | year | decade | language | style"
...
**Identification**: `fingerprint`(0008, Chromaprint over the first 120s), `fingerprint_duration`(0008, full-file length), `fingerprint_attempted_at`(0029, unix seconds, 0 = never attempted; failures are persisted too so scans don't re-run ffmpeg every round), `isrc`(0009)
