...
The default ordering rule is `position ASC, updated_at DESC` (when positions are equal, the most recently updated comes first); pinned playlists (see below) always sort ahead of this rule.

### Pinning (SetPinned)

`PUT /api/v1/playlists/{id}/pin`, request body `{"pinned": true}`

The nullable `pinned_at` (`DATETIME`) column on the `playlists` table doubles as both the "is pinned" flag and the "pin time": `NULL` means unpinned, and a non-`NULL` value is the pin timestamp. `PlaylistService.SetPinned` writes `time.Now()` when pinning and writes back `NULL` when unpinning.

At query time, the repository's `List()` first appends `ORDER BY p.pinned_at IS NULL ASC, p.pinned_at DESC`, then the existing `position ASC, updated_at DESC` (or a client-specified order). Because squirrel's `OrderBy` accumulates rather than overwrites, pinning always takes effect as the highest-priority prefix, fully decoupled from the ordering fields that follow it.

Unlike built-in playlist protection (Section 2), pinning is **not** subject to the built-in update restriction — Favorites and Radio Favorites can be pinned too. The pin state is exposed via the `pinned_at` field returned by `GET /playlists`, which the client uses to render the pin badge and ordering in the "Library → All Playlists" list, with no separate filter query parameter needed.
