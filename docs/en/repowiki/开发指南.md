...
The frontend code is located in `clients/player/` (a separate repository); you need to clone it or initialize the submodule first before development.
...
Frontend build artifacts are uniformly output to `clients/player-build/` (not `clients/player/build/`).
...
Multi-stage build: `ghcr.io/songloft-org/songloft-base-golang:1.26-alpine` (build stage, accelerated by BuildKit cache mounts) → `ghcr.io/songloft-org/songloft-base-alpine:3.24` (runtime, with ffmpeg, ALSA, and ca-certificates). The release workflow mirrors the upstream Docker Hub base images to GHCR before starting the multi-platform build.
...
When building the full version, the frontend artifact must be located at `clients/player-build/web-embedded/` (not `clients/player/build/`), embedded by `internal/app/embed.go`.
