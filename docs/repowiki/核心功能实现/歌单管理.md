...
- [internal/models/models.go](https://github.com/songloft-org/songloft/blob/main/internal/models/models.go) -- Playlist / PlaylistSong 模型定义、类型常量、CanAddSong 校验
- [internal/database/migrations/0001_init.sql](https://github.com/songloft-org/songloft/blob/main/internal/database/migrations/0001_init.sql) -- playlists / playlist_songs 表结构与约束
- [internal/database/playlist_repository.go](https://github.com/songloft-org/songloft/blob/main/internal/database/playlist_repository.go) -- 歌单仓储（CRUD、AutoCreate、BatchDelete 等）
...
- [internal/services/playlist_service.go](https://github.com/songloft-org/songloft/blob/main/internal/services/playlist_service.go) -- 歌单服务层
- [internal/services/backup_service.go](https://github.com/songloft-org/songloft/blob/main/internal/services/backup_service.go) -- 歌单备份与恢复
- [internal/services/song_service.go](https://github.com/songloft-org/songloft/blob/main/internal/services/song_service.go) -- 扫描完成后自动创建歌单触发逻辑
- [internal/handlers/playlist.go](https://github.com/songloft-org/songloft/blob/main/internal/handlers/playlist.go) -- 歌单 HTTP handler
- [internal/handlers/backup.go](https://github.com/songloft-org/songloft/blob/main/internal/handlers/backup.go) -- 备份导入导出 handler
- [internal/handlers/scan.go](https://github.com/songloft-org/songloft/blob/main/internal/handlers/scan.go) -- 自动创建歌单开关配置 handler
...
默认排序规则为 `position ASC, updated_at DESC`（position 相同时最近更新的排前面）；置顶歌单（见下）始终排在这套规则之前。

### 歌单置顶（SetPinned）

`PUT /api/v1/playlists/{id}/pin`，请求体 `{"pinned": true}`

`playlists` 表的 `pinned_at` 列（可空 `DATETIME`）同时承担"是否置顶"和"置顶时间"两个语义：`NULL` 表示未置顶，非 `NULL` 即置顶时间戳。`PlaylistService.SetPinned` 置顶时写入 `time.Now()`，取消置顶时写回 `NULL`。

仓储层 `List()` 在查询时先叠加 `ORDER BY p.pinned_at IS NULL ASC, p.pinned_at DESC`，再拼接原有的 `position ASC, updated_at DESC`（或客户端指定的排序）；squirrel 的 `OrderBy` 是累加而非覆盖，因此置顶始终作为最高优先级前缀生效，与后面的排序字段完全解耦。

与内置歌单保护（第 2 节）不同，置顶**不受**内置歌单更新限制——收藏、电台收藏同样可以被置顶。置顶状态通过 `GET /playlists` 返回的 `pinned_at` 字段暴露，客户端据此在"曲库-全部歌单"列表里渲染置顶标签与排序，无需额外的过滤查询参数。
