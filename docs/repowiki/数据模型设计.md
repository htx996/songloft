...
当前 schema 共 **8 张表**：`songs`、`playlists`、`playlist_songs`、`play_history`、`configs`、`auth_tokens`、`js_plugins`、`plugin_storage`，通过 35 个迁移文件从初始版本演进到当前状态。核心表的 `updated_at` 列均由 SQLite TRIGGER 自动维护。
...
**识别**：`fingerprint`(0008, Chromaprint 前 120 秒采样), `fingerprint_duration`(0008, 全片时长), `fingerprint_attempted_at`(0029, unix 秒，0=未尝试；失败也落库，避免每轮扫描重跑 ffmpeg), `isrc`(0009)
