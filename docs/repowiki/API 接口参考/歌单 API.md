...
置顶/取消置顶歌单。置顶歌单在 `GET /playlists` 列表中始终排最前，多个置顶歌单按置顶时间倒序（最近置顶的排最前）。内置歌单（收藏、电台收藏）同样允许置顶，不做特殊限制。

- **认证**: Bearer Token
- **路径参数**: `id`（int，歌单 ID）
- **请求体**: `{"pinned": true}`（bool，必填）
- **200**: 返回更新后的 `Playlist` 对象（`pinned_at` 字段反映置顶状态）
- **400**: 无效的歌单ID或请求数据 | **404**: 歌单不存在 | **500**: 服务器错误
